...
export async function getKycPhone(): Promise<string | null> {
  const response =
    await apiClient<{
      success: boolean;
      phone: string | null;
...
export async function requestKycPhoneOtp(
  phone: string,
...
export async function verifyKycPhoneOtp(
  input: {
    challengeId: string;
    otp: string;
  }
): Promise<PhoneOtpVerification> {
  const response =
    await apiClient<{
      success: boolean;
...
export async function validateNidDocuments(
  input: {
    phoneChallengeId: string;
    frontImage: File;
    backImage: File;
  ...
  const response =
    await apiClient<{
      success: boolean;
      validation: NIDDocumentValidation;
    }>(
...
export async function verifyKycDeviceBiometric(): Promise<DeviceBiometricProof> {
  if (
    ...
    throw new Error(
      "Device biometrics are not supported on this browser."
    );
  }
...
export async function getCurrentEKYC(): Promise<EKYCVerification | null> {
  const response =
    await apiClient<CurrentResponse>(
      "/ekyc/verifications/current"
    );
...
export async function submitEKYC(
  ...
  body.append(
    "livenessEvidence",
    input.liveness.video
  );

  body.append(
    "livenessSessionId",
    input.liveness.session
      .sessionId
  );
  ...
  body.append(
    "livenessStartedAt",
    input.liveness.startedAt
  );

  body.append(
    "livenessCompletedAt",
    input.liveness.completedAt
  );
  ...
  body.append(
    ...
    input.documentValidationId
  );

  if (
    input.biometricSessionId
  ) {
    body.append(
      "biometricSessionId",
      input.biometricSessionId
...
export async function getAdminEKYCList(
  input: {
    status?: EKYCStatus;
    search?: string;
    page?: number;
  ...
  return apiClient<AdminListResponse>(
    `/admin/ekyc/verifications?${query.toString()}`
  );
}

export async function getAdminEKYCDetails(
  id: string
): Promise<{
...
export async function getAdminEKYCDocuments(
  id: string
): Promise<EKYCDocuments> {
  const response =
    await apiClient<{
      success: boolean;
...
export async function decideAdminEKYC(
  ...
  await apiClient(
    `/admin/ekyc/verifications/${id}/decision`,
    {
      method: "PATCH",
      headers: {
        "Content-Type":
...
export async function rerunAdminEKYC(
  id: string
): Promise<void> {
  await apiClient(
    `/admin/ekyc/verifications/${id}/rerun`,
    {
      method: "POST",
    }
  );
