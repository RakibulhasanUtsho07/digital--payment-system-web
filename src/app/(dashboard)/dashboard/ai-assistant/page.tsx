...
import {
  ...
  sendUserAiMessage,
  type UserAiAction,
  type UserAiConfidence,
  type UserAiConversation,
  type UserAiFact,
  type UserAiSource,
  type UserAiVerification,
} from "@/lib/api/cofferUserAiApi";
...
const QUICK_PROMPTS = [
  {
    title: "Wallet status",
    description:
      "Check my current wallet status and balance summary.",
    prompt:
      "What is my wallet status and current balance summary?",
    icon: WalletCards,
  },
  {
    title: "KYC status",
    description:
      "Explain my current KYC verification status.",
    prompt:
      "What is my current KYC status and what should I do next?",
    icon: FileCheck2,
  },
  {
    title: "Payment diagnosis",
    description:
      "Investigate one of my payments using a payment reference.",
    prompt:
      "Why did this payment fail? Use only my verified account records.",
    icon: CreditCard,
  },
  {
    title: "Transaction lookup",
    description:
      "Check one of my wallet transactions by transaction ID.",
    prompt:
      "Show the verified details for this transaction. If it failed, do not guess the exact cause.",
    icon: ArrowLeftRight,
  },
  {
    title: "Receipt lookup",
    description:
      "Explain one of my saved receipts using its receipt ID.",
    prompt:
      "Show the verified details for this receipt from my account.",
    icon: ReceiptText,
  },
  {
    title: "Security summary",
    description:
      "Review my security score, 2FA and recent safe security signals.",
    prompt:
      "Give me my verified account security summary. Do not expose session IDs, IP addresses, secrets or OTPs.",
    icon: ShieldAlert,
  },
] as const;

const verificationClasses:
  Record<UserAiVerification, string> = {
    verified:
...
const confidenceClasses:
  Record<UserAiConfidence, string> = {
    high:
      "border-emerald-500/20 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300",
    medium:
      "border-amber-500/20 bg-amber-500/10 text-amber-700 dark:text-amber-300",
    ...
    <main className="relative flex min-h-[78vh] items-center justify-center overflow-hidden bg-background px-4 text-foreground">
      ...
      >
        ...
        <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted-foreground">
          This personal AI workspace is available only to an authenticated User account.
        </p>
      </motion.section>
    </main>
  );
...
export default function UserAiAssistantPage() {
  ...
  if (role !== "user") {
    return <UserAiNotFoundState />;
  }

  return (
    <UserAiAssistantContent
      userName={user.name}
    />
  );
}
...
function UserAiAssistantContent({
  userName,
}: {
  userName?: string;
}) {
  ...
  const loadConversations =
    useCallback(
      async (
        ...
        try {
          const response =
            await getUserAiConversations(30);
    ...
    useCallback(
      async (
        ...
        if (
          ...
          setError(
            "Reference may contain only letters, numbers, underscore, and hyphen.",
          );
          return;
        }
      ...
      <div className="mx-auto w-full max-w-[1540px] space-y-5">
          ...
          <div className="relative z-10 flex flex-col gap-7 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-4xl">
              ...
              <h1 className="mt-4 max-w-3xl text-2xl font-black tracking-[-0.04em] sm:text-3xl lg:text-[38px] lg:leading-[1.08]">
                Hi {firstName(userName)}, ask Coffer AI about your account
              </h1>

              <p className="mt-3 max-w-3xl text-sm leading-6 text-violet-100/75">
                Check your own wallet, KYC, payments, transactions, receipts and security summary through a read-only, backend-authorized AI workspace. Coffer AI never trusts a client-supplied role or user ID as authority.
              </p>
        ...
        <section className="grid min-h-[680px] gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
          ...
          <aside className="overflow-hidden rounded-[28px] border border-border bg-card shadow-sm">
            ...
            <div className="max-h-[610px] overflow-y-auto p-2.5">
              {loadingConversations ? (
                ...
                <div className="rounded-2xl border border-dashed border-border p-5 text-center">
                  ...
                  <p className="mt-1 text-[10px] leading-5 text-muted-foreground">
                    Ask about your wallet, KYC, payments, transactions, receipts or security.
                  </p>
                </div>
              ) : (
          ...
          {/* CHAT */}
          <article className="flex min-h-[680px] min-w-0 flex-col overflow-hidden rounded-[28px] border border-border bg-card shadow-sm">
            <div className="flex flex-col gap-3 border-b border-border bg-violet-500/[0.04] p-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              {loadingMessages ? (
                ...
                <div className="mx-auto flex min-h-[500px] max-w-xl flex-col items-center justify-center text-center">
                  ...
                  <p className="mt-3 max-w-md text-xs leading-6 text-muted-foreground">
                    Ask about your own wallet, KYC, payment/transaction status, saved receipts or account security. If the backend does not have enough evidence, Coffer AI should say that it cannot verify the exact cause.
                  </p>
                </div>
              ) : (
                <div
                ...
                >
                  ...
                  {sending ? (
                    <div className="flex items-center gap-3 text-[10px] font-bold text-muted-foreground">
                      ...
                      Checking your authorized account records…
                    </div>
                  ) : null}
                </div>
              )}
            ...
            <AnimatePresence>
              {error ? (
                <motion.div
                  ...
                  role="alert"
                  className="mx-4 mb-3 flex items-start gap-3 rounded-2xl border border-rose-500/20 bg-rose-500/[0.06] p-3 text-rose-700 dark:text-rose-300 sm:mx-5"
                >
            ...
            >
              <div className="mb-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
                <label className="flex h-11 items-center gap-2 rounded-xl border border-border bg-background px-3 focus-within:border-violet-400 focus-within:ring-4 focus-within:ring-violet-500/10">
                  ...
                  <input
                    ...
                    placeholder="Optional payment / transaction / receipt ID"
              ...
              <div className="flex items-end gap-2 rounded-[20px] border border-border bg-background p-2 focus-within:border-violet-400 focus-within:ring-4 focus-within:ring-violet-500/10">
                <textarea
                  ...
                  placeholder="Ask about wallet, KYC, payments, transactions, receipts or security…"
                  rows={1}
                  maxLength={4000}
              ...
              <div className="mt-3 flex flex-col gap-2 text-[8px] font-bold text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
                ...
                <span className="inline-flex items-center gap-1">
                  ...
                  Coffer AI cannot execute transfers, refunds or wallet changes
                </span>
              </div>
            </form>
          </article>
